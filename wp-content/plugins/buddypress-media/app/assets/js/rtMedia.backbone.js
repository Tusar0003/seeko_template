...
jQuery( function( $ ) {
	...
	rtMedia.Gallery = Backbone.Collection.extend( {
		...
		url: function() {
			...
			if ( temp.indexOf( '/' + rtmedia_media_slug + '/' ) == -1 ) {
				...
				if ( temp.indexOf( 'pg/index.html' ) == -1 ) {
					url = temp;
				} else {
					url = window.location.pathname.substr( 0, window.location.pathname.lastIndexOf( 'pg/index.html' ) );
				}
			}
			if ( ! upload_sync && nextpage >= 1 ) {
				if ( url.substr( url.length - 1 ) != 'https://seeko.seventhqueen.com/' ) {
					url += 'https://seeko.seventhqueen.com/';
				}
	...
	$( document ).on( 'click', '#rtmedia_show_all_comment', function() {
		...
		if ( window.location.pathname.indexOf( rtmedia_media_slug ) != -1 ) {
			var tempNext = window.location.pathname.substring( window.location.pathname.lastIndexOf( 'pg/index.html' ) + 5, window.location.pathname.lastIndexOf( 'https://seeko.seventhqueen.com/' ) );
